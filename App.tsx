...
import { Plus, Search, Filter, Trash2, Calendar, Settings, Info, Heart, List } from 'lucide-react';
...
const App: React.FC = () => {
  const [promises, setPromises] = useLocalStorage<PromiseEntry[]>('promises', []);
  ...
  const handleSavePromise = (data: Omit<PromiseEntry, 'id' | 'createdAt'> & { id?: string }) => {
    if (data.id) {
      // Edit existing
  ...
  const handleEditClick = (promise: PromiseEntry) => {
  ...
  return (
    <div className="min-h-screen pb-32 bg-gray-50 dark:bg-black overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 bg-gray-50/80 dark:bg-black/80 backdrop-blur-md px-6 pt-12 pb-6 flex flex-col space-y-4">
        <div className="flex justify-between items-end">
          ...
          <button 
            ...
            className="p-2.5 bg-blue-500 text-white rounded-full shadow-lg shadow-blue-500/20 active:scale-90 transition-transform"
          >
      ...
      {/* Bottom Bar / Nav (Simulated iOS style) */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-sm z-50">
        <nav className="bg-white/90 dark:bg-neutral-900/90 ios-blur p-2 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-xl flex items-center justify-around">
          ...
          <button 
            ...
            onClick={() => alert("Privacy settings and data management features are coming soon. Your data is currently stored only in your browser.")}
          >
      ...
      <AddPromiseModal 
        isOpen={isModalOpen} 
        onClose={() => { setIsModalOpen(false); setEditingPromise(null); }}
